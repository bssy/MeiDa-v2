...
public class MyImgScroll extends ViewPager
{
	 ...
	 * @param adtexts
	 */
	public void start(Activity mainActivity, List<String> imgUrls, int scrollTime, LinearLayout ovalLayout, int ovalLayoutId, int ovalLayoutItemId, int focusedId, int normalId,
			List<String> adtexts)
	{
		if(mainActivity == null)
		{
			new NullPointerException("Activity 对象参数不能为空!");
			return;
		}
		mActivity = mainActivity;
		this.imgUrls = imgUrls;
		mListViews = initViewPager(mainActivity,imgUrls);
		mScrollTime = scrollTime;
		// 设置圆点
		setOvalLayout(ovalLayout, ovalLayoutId, ovalLayoutItemId, focusedId, normalId);
		this.setAdapter(new MyPagerAdapter());// 设置适配器
		if( scrollTime != 0 && imgUrls.size() > 1 )
		{
			// 设置滑动动画时间 ,如果用默认动画时间可不用 ,反射技术实现
			new FixedSpeedScroller(mActivity).setDuration(this, 700);
	 ...
	 * 加载网络图片 初始化图片
	 */
	private List<View> initViewPager(Activity mainActivity, List<String> imgUrls)
	{
		...
		for ( int i = 0; i < imgUrls.size(); i++ )
		{
			...
			if( imgUrls.get(i) != null )
				// 传入监听 并为指定控件加载图片
				mImageLoader.get(imgUrls.get(i), listener);
	...
	// 适配器 //循环设置
	private class MyPagerAdapterForLoadImg extends PagerAdapter
	{
		public void finishUpdate(View arg0)
		{
		}
		...
		public Object instantiateItem(View v, int i)
		{
			if( ((ViewPager) v).getChildCount() == imgUrls.size() )
			{
				((ViewPager) v).removeView(mListViews.get(i % mListViews.size()));
			}
			((ViewPager) v).addView(mListViews.get(i % mListViews.size()), 0);
